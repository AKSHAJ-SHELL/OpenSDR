...
| CRM connections | `hubspot` (private-app token) \| `salesforce` (client-credentials OAuth at the org's My Domain; `instance_url` through the SSRF guard at write AND use). Credentials write-only at the API, Fernet-encrypted at rest; field map = per-connection JSONB overlay on `crm/mapping.py DEFAULT_MAPS` | `/crm/connections` (admin); migration `0019` (M5.2) |
| CRM import doctrine | dry-run default (`POST .../import`); commits flow through the SAME ingest gate as CSV, then CRM-wins updates limited to `CRM_OWNED_LEAD_FIELDS` (first/last/title/linkedin/timezone — blank CRM fields never clear), links via `crm_links` (unique both ways per connection), optional score-and-enroll of the verified subset through `scoring/enroll.py` (the one enrollment path) | `crm/sync.py`, `crm/mapping.py` (M5.2) |
| CRM outbound push | beat `crm_sync_tick` every 900s + `POST .../sync`; sends/classified replies/meetings → linked contacts only, at-most-once past the per-connection watermark (failures tallied on the run, never retried forever); every run recorded in `crm_sync_runs` | `workers/tasks.py crm_sync_tick`, `crm/sync.py push_activity` (M5.2) |
